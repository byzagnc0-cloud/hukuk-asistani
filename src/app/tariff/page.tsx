...
import { Plus, Search, Trash2, Edit2, Loader2, Receipt, FolderPlus } from 'lucide-react'
...
import { TariffItem, TariffCategory, DEFAULT_TARIFF_CATEGORIES } from '@/lib/types'
...
export default function TariffPage() {
  ...
  async function fetchAll() {
    ...
    const [{ data: cats }, { data: its }] = await Promise.all([
      supabase.from('tariff_categories').select('*').eq('user_id', user!.id).order('sort_order').order('name'),
      supabase.from('tariff_items').select('*').eq('user_id', user!.id).order('created_at', { ascending: false }),
    ])
    setCategories(cats ?? [])
    setItems(its ?? [])
    setFetching(false)
  ...
  async function ensureDefaultCategories(existingCats: TariffCategory[]) {
    if (existingCats.length > 0) return
    const defaults = DEFAULT_TARIFF_CATEGORIES.map((name, i) => ({
      user_id: user!.id,
      name,
      sort_order: i,
    }))
    await supabase.from('tariff_categories').insert(defaults)
    fetchAll()
  }
  ...
  function openEdit(item: TariffItem) {
  ...
  async function handleSave() {
    ...
    if (editingId) {
      const { error } = await supabase.from('tariff_items').update(payload).eq('id', editingId)
      if (error) { addToast('Güncellenemedi', 'error'); setSaving(false); return }
      addToast('Güncellendi', 'success')
    } else {
      const { data, error } = await supabase.from('tariff_items').insert(payload).select().single()
  ...
  async function handleDelete(id: string) {
    await supabase.from('tariff_items').delete().eq('id', id)
  ...
  async function addCategory() {
    ...
    await supabase.from('tariff_categories').insert({
      user_id: user!.id,
      name: catName,
    ...
    <AppLayout>
      <div className="max-w-5xl mx-auto animate-fadeIn">
        <div className="page-header">
          <div>
            <h1 className="page-title">Asgari Ücret Tarifesi</h1>
            <p className="text-sm text-gray-500 mt-0.5">{items.length} kayıt — AAÜT, harç ve tarife notları</p>
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="card text-center py-16">
            ...
            <p className="text-sm text-gray-400 mt-1">Tarife, harç veya ücret bilgilerini manuel olarak ekleyin</p>
          </div>
        ) : (
      ...
      {/* Item Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Düzenle' : 'Tarifeye Ekle'} size="xl">
        <div className="space-y-4">
          <div>
            <label className="label">Başlık *</label>
            <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Tarife veya harç adı..." className="input-field" />
          </div>
          <div>
          ...
          <div>
            <label className="label">İçerik / Tutarlar</label>
            <textarea value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} rows={8} placeholder="Tarife tutarları, oranlar, notlar..." className="input-field resize-none font-mono text-sm" />
          </div>
          {savedEntityId && (
            <div>
              <label className="label">Dosya Ekleri (PDF, Word, Excel, UDF)</label>
              <FileUpload entityType="tariff_item" entityId={savedEntityId} userId={user!.id} />
            </div>
          )}
      ...
      <Modal isOpen={catModalOpen} onClose={() => setCatModalOpen(false)} title="Yeni Kategori" size="sm">
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={catName} onChange={e => setCatName(e.target.value)} placeholder="Örn: Harçlar" className="input-field" />
          </div>
          <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
